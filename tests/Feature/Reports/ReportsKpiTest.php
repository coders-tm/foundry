...
beforeEach(function () {
    ...
    for ($monthsAgo = 11; $monthsAgo >= 0; $monthsAgo--) {
        ...
        for ($i = 0; $i < $subsCount; $i++) {
            ...
            $rand = rand(1, 100);
            if ($rand <= 70) {
                $subscription = \Foundry\Models\Subscription::factory()->create(['user_id' => $user->id, 'plan_id' => $plan->id, 'status' => 'active', 'created_at' => $createdAt]);
            } elseif ($rand <= 85) {
                $subscription = \Foundry\Models\Subscription::factory()->create(['user_id' => $user->id, 'plan_id' => $plan->id, 'status' => 'active', 'trial_ends_at' => now()->addDays(rand(1, 14)), 'created_at' => $createdAt]);
            } else {
                $subscription = \Foundry\Models\Subscription::factory()->create(['user_id' => $user->id, 'plan_id' => $plan->id, 'status' => 'canceled', 'cancels_at' => $createdAt->copy()->addMonths(rand(1, 3)), 'created_at' => $createdAt]);
            }
            \Foundry\Models\Order::factory()->create(['customer_id' => $user->id, 'orderable_id' => $subscription->id, 'orderable_type' => $subscription->getMorphClass(), 'payment_status' => 'paid', 'grand_total' => $plan->price, 'created_at' => $createdAt]);
        }
    }
    for ($monthsAgo = 11; $monthsAgo >= 0; $monthsAgo--) {
