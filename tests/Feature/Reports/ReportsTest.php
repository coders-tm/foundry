...
$seedTestData = function () {
    for ($monthsAgo = 11; $monthsAgo >= 0; $monthsAgo--) {
        $count = rand(2, 5);
        \App\Models\User::factory()->count($count)->create(['created_at' => now()->subMonths($monthsAgo)->addDays(rand(0, 28))]);
    }
    $monthlyPlan = \Foundry\Models\Subscription\Plan::factory()->create(['price' => 29.99, 'interval' => 'month', 'interval_count' => 1]);
    $yearlyPlan = \Foundry\Models\Subscription\Plan::factory()->create(['price' => 299.99, 'interval' => 'year', 'interval_count' => 1]);
    ...
    for ($monthsAgo = 11; $monthsAgo >= 0; $monthsAgo--) {
        ...
        for ($i = 0; $i < $subsCount; $i++) {
            ...
            $rand = rand(1, 100);
            if ($rand <= 70) {
                \Foundry\Models\Subscription::factory()->create(['user_id' => $user->id, 'plan_id' => $plan->id, 'status' => 'active', 'created_at' => $createdAt]);
            } elseif ($rand <= 85) {
                \Foundry\Models\Subscription::factory()->create(['user_id' => $user->id, 'plan_id' => $plan->id, 'status' => 'active', 'trial_ends_at' => now()->addDays(rand(1, 14)), 'created_at' => $createdAt]);
            } else {
                \Foundry\Models\Subscription::factory()->create(['user_id' => $user->id, 'plan_id' => $plan->id, 'status' => 'canceled', 'cancels_at' => $createdAt->copy()->addMonths(rand(1, 3)), 'created_at' => $createdAt]);
            }
        }
    }
    for ($monthsAgo = 11; $monthsAgo >= 0; $monthsAgo--) {
        ...
        for ($i = 0; $i < $ordersCount; $i++) {
            \Foundry\Models\Order::factory()->create(['customer_id' => $users->random()->id, 'payment_status' => rand(1, 100) <= 95 ? 'paid' : 'pending', 'grand_total' => rand(50, 500) + (rand(0, 99) / 100), 'created_at' => now()->subMonths($monthsAgo)->addDays(rand(0, 28))]);
        }
    }
};

it('subscription metrics endpoint works', function () use ($seedTestData) {
...
it('order metrics endpoint works', function () use ($seedTestData) {
...
it('customer metrics endpoint works', function () use ($seedTestData) {
...
it('customer chart endpoint works', function () use ($seedTestData) {
...
it('mrr chart endpoint works', function () use ($seedTestData) {
...
it('churn chart endpoint works', function () use ($seedTestData) {
...
it('revenue breakdown chart endpoint works', function () use ($seedTestData) {
...
it('arpu chart endpoint works', function () use ($seedTestData) {
...
it('plan distribution chart endpoint works', function () use ($seedTestData) {
...
it('cache clear endpoint works', function () use ($seedTestData) {
...
it('date filtering works', function () use ($seedTestData) {
    $response = $this->getJson('/admin/reports/metrics?category=retention&start_date=2025-01-01&end_date=2025-11-24');
    
    $response->assertStatus(200);
});

it('compare flag returns comparisons', function () use ($seedTestData) {
...
it('chart type validation', function () use ($seedTestData) {
...
it('new chart type validation', function () use ($seedTestData) {
