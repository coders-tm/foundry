...
$createSubscriptionWithOpenInvoice = function (bool $pastDue = true) use ($createPlan) {
    ...
    $subscription = \Foundry\Models\Subscription::factory()->create([
        'plan_id' => $plan->id,
        'status' => $pastDue ? \Foundry\Models\Subscription::EXPIRED : \Foundry\Models\Subscription::PENDING,
        'starts_at' => now(), 'expires_at' => now()->addMonth(),
    ]);
    $start = \Carbon\Carbon::now()->subMonth()->startOfDay();
    $end = (clone $start)->addMonth();
    $description = $start->format('M d, Y').' - '.$end->format('M d, Y');
    $order = \Foundry\Models\Order::create([
        'customer_id' => $subscription->user_id, 'orderable_id' => $subscription->id, 'orderable_type' => \Foundry\Models\Subscription::class,
...
it('mark as paid activates subscription and sets period based on subscription when paid', function () use ($createSubscriptionWithOpenInvoice) {
    ...
    $this->assertEquals(\Foundry\Models\Subscription::ACTIVE, $subscription->status);
