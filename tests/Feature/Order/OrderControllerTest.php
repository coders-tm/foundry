...
it("store creates item and redirects", function () {
    ...
    $data = ["customer_id" => $user->id, "status" => \Foundry\Enum\OrderStatus::PENDING->value, "note" => "Test order note", "line_items" => [["title" => "Product 1", "price" => 100, "quantity" => 1]], "sub_total" => 100, "grand_total" => 100];
    ...
    $order = \Foundry\Models\Order::where("customer_id", $user->id)->first();
...
it("store marks as paid if payment method provided", function () {
    $user = \Foundry\Models\User::factory()->create();
    $data = ["customer_id" => $user->id, "status" => \Foundry\Enum\OrderStatus::PENDING->value, "payment_method" => "manual", "line_items" => [["title" => "Product 1", "price" => 100, "quantity" => 1]]];
...
it("store sends invoice if invoice data provided", function () {
    ...
    \Foundry\Models\Notification::factory()->create(["type" => "user:invoice-sent", "subject" => "Invoice for Order {{ $order->number }}", "content" => "Hello"]);
    $user = \Foundry\Models\User::factory()->create();
    $data = ["customer_id" => $user->id, "status" => \Foundry\Enum\OrderStatus::PENDING->value, "invoice_data" => ["to" => "custom@example.com", "subject" => "Custom Subject", "message" => "Custom Message"], "line_items" => [["title" => "Product 1", "price" => 100, "quantity" => 1]]];
    $this->actingAs($this->admin, "admin")->post(route("admin.orders.store"), $data)->assertRedirect();
    $order = \Foundry\Models\Order::where("customer_id", $user->id)->first();
    \Illuminate\Support\Facades\Notification::assertSentTo(new \Foundry\Models\Order\Customer(["id" => $order->customer_id]), \Foundry\Notifications\OrderInvoiceNotification::class, fn ($n, $c, $notifiable) => $notifiable->email === "custom@example.com");
    $this->assertDatabaseHas("logs", ["type" => "invoice_sent", "logable_id" => $order->id, "logable_type" => "Order"]);
});

it("update modifies item", function () {
    $item = \Foundry\Models\Order::factory()->create(["status" => \Foundry\Enum\OrderStatus::PENDING]);
    $data = ["status" => \Foundry\Enum\OrderStatus::COMPLETED->value, "line_items" => [["title" => "Updated Product", "price" => 150, "quantity" => 1]]];
...
it("cancel marks order as cancelled", function () {
    ...
    $this->assertEquals(\Foundry\Enum\OrderStatus::CANCELLED, $item->refresh()->status);
});

it("mark as paid updates payment status", function () {
    $item = \Foundry\Models\Order::factory()->create(["payment_status" => \Foundry\Enum\PaymentStatus::PAYMENT_PENDING]);
...
it("send invoice sends notification", function () {
    \Illuminate\Support\Facades\Notification::fake();
    $user = \Foundry\Models\User::factory()->create(["email" => "original@example.com"]);
    $item = \Foundry\Models\Order::factory()->create(["customer_id" => $user->id]);
    \Foundry\Models\Notification::factory()->create(["type" => "user:invoice-sent", "subject" => "Invoice for Order {{ $order->number }}", "content" => "Hello"]);
    $this->actingAs($this->admin, "admin")->post(route("admin.orders.send-invoice", $item))->assertRedirect();
    \Illuminate\Support\Facades\Notification::assertSentTo(new \Foundry\Models\Order\Customer(["id" => $item->customer_id]), \Foundry\Notifications\OrderInvoiceNotification::class);
    ...
    \Illuminate\Support\Facades\Notification::assertSentTo(new \Foundry\Models\Order\Customer(["id" => $item->customer_id]), \Foundry\Notifications\OrderInvoiceNotification::class, fn ($n, $c, $notifiable) => $notifiable->email === $customEmail);
});
...
it("refund processes refund", function () {
    $user = \Foundry\Models\User::factory()->create();
    $item = \Foundry\Models\Order::factory()->create(["customer_id" => $user->id, "payment_status" => \Foundry\Enum\PaymentStatus::PAID, "status" => \Foundry\Enum\OrderStatus::COMPLETED, "grand_total" => 100, "paid_total" => 100]);
    $item->payments()->create(["transaction_id" => "txn_123", "amount" => 100, "status" => \Foundry\Models\Payment::STATUS_COMPLETED, "processed_at" => now(), "currency" => "USD", "provider" => "manual"]);
    $this->actingAs($this->admin, "admin")->post(route("admin.orders.refund", $item), ["to_wallet" => true])->assertRedirect();
    $this->assertEquals(\Foundry\Enum\OrderStatus::REFUNDED, $item->refresh()->status);
});

it("show subscription invoice includes line items", function () {
    $user = \Foundry\Models\User::factory()->create();
    $plan = \Foundry\Models\Subscription\Plan::factory()->create(["price" => 19.99, "trial_days" => 0]);
    $this->actingAs($this->admin, "admin")->post(route("admin.subscriptions.store"), ["user_id" => $user->id, "plan" => $plan->id, "starts_at" => now()->toDateTimeString(), "generate_invoice" => true, "mark_as_paid" => true, "payment_method" => "manual"])->assertSessionHasNoErrors()->assertRedirect();
    $subscription = \Foundry\Foundry::$subscriptionModel::where("user_id", $user->id)->first();
...
it("store with fixed discount creates discount line", function () {
    $user = \Foundry\Models\User::factory()->create();
    $data = ["customer_id" => $user->id, "status" => \Foundry\Enum\OrderStatus::PENDING->value, "line_items" => [["title" => "Product 1", "price" => 100, "quantity" => 1]], "discount" => ["type" => "fixed_amount", "value" => 10, "description" => "Test discount"], "sub_total" => 100, "discount_total" => 10, "grand_total" => 90];
    $this->actingAs($this->admin, "admin")->post(route("admin.orders.store"), $data)->assertSessionHasNoErrors()->assertRedirect();
    $order = (\Foundry\Foundry::$orderModel)::where("customer_id", $user->id)->first();
...
it("store with percentage discount creates discount line", function () {
    $user = \Foundry\Models\User::factory()->create();
    $data = ["customer_id" => $user->id, "status" => \Foundry\Enum\OrderStatus::PENDING->value, "line_items" => [["title" => "Product 1", "price" => 100, "quantity" => 1]], "discount" => ["type" => "percentage", "value" => 10, "description" => "10% off"], "sub_total" => 100, "discount_total" => 10, "grand_total" => 90];
    $this->actingAs($this->admin, "admin")->post(route("admin.orders.store"), $data)->assertSessionHasNoErrors()->assertRedirect();
    $order = (\Foundry\Foundry::$orderModel)::where("customer_id", $user->id)->first();
    $this->assertNotNull($order);
    $this->assertEquals(10, $order->discount_total);
...
it("update with discount creates or updates discount line", function () {
    $user = \Foundry\Models\User::factory()->create();
    $order = \Foundry\Models\Order::factory()->create(["customer_id" => $user->id, "status" => \Foundry\Enum\OrderStatus::PENDING->value, "discount_total" => 0, "sub_total" => 100, "tax_total" => 0, "grand_total" => 100]);
...
it("show order includes all necessary relationships", function () {
    $user = \Foundry\Models\User::factory()->create();
    $order = \Foundry\Models\Order::create(["customer_id" => $user->id, "status" => \Foundry\Enum\OrderStatus::PENDING->value, "line_items" => [["title" => "Product 1", "price" => 100, "quantity" => 1, "taxable" => true]], "discount" => ["type" => "fixed_amount", "value" => 20, "description" => "Test discount"], "tax_lines" => [["label" => "Sales Tax", "rate" => 10]], "contact" => ["email" => "test@example.com", "phone_number" => "1234567890"]]);
...
it("order create calculates correctly with compounded taxes and discount", function () {
    $user = \Foundry\Models\User::factory()->create();
    $order = \Foundry\Models\Order::create(["customer_id" => $user->id, "status" => \Foundry\Enum\OrderStatus::PENDING->value, "line_items" => [["title" => "Product 1", "price" => 100, "quantity" => 1, "taxable" => true], ["title" => "Product 2", "price" => 100, "quantity" => 1, "taxable" => true]], "discount" => ["type" => "percentage", "value" => 10], "tax_lines" => [["label" => "GST", "rate" => 10, "type" => "normal"], ["label" => "Cess", "rate" => 5, "type" => "compounded"]]]);
...
it("order update recalculates correctly when data changes", function () {
    $user = \Foundry\Models\User::factory()->create();
    $order = \Foundry\Models\Order::create(["customer_id" => $user->id, "line_items" => [["title" => "Initial Item", "price" => 100, "quantity" => 1]], "collect_tax" => false]);
...
it("store endpoint calculates correctly", function () {
    $user = \Foundry\Models\User::factory()->create();
    $data = ["customer_id" => $user->id, "status" => \Foundry\Enum\OrderStatus::PENDING->value, "line_items" => [["title" => "Product A", "price" => 100, "quantity" => 2, "taxable" => true]], "tax_lines" => [["label" => "VAT", "rate" => 20, "type" => "normal"]], "discount" => ["type" => "fixed_amount", "value" => 50]];
    $this->actingAs($this->admin, "admin")->post(route("admin.orders.store"), $data)->assertRedirect();
    $order = (\Foundry\Foundry::$orderModel)::where("customer_id", $user->id)->latest()->first();
    $order->load(["tax_lines", "line_items"]);
    $this->assertEquals(200.00, (float) $order->sub_total);
    ...
    $this->assertEquals(30.00, (float) \Foundry\Models\Order\TaxLine::where("taxable_id", $order->id)->first()->amount);
});
